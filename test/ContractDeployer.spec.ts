...
xdescribe("ContractDeployer tests", function () {
  ...
  const EOA = ethers.utils.getAddress(
    "0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef"
  );
  const RANDOM_ADDRESS = ethers.utils.getAddress(
    "0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbee1"
  );
  const RANDOM_ADDRESS_2 = ethers.utils.getAddress(
    "0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbee2"
  );
  const RANDOM_ADDRESS_3 = ethers.utils.getAddress(
    "0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbee3"
  );
  ...
  before(async () => {
    ...
    await setCode(
      DEPLOYER_SYSTEM_CONTRACT_ADDRESS,
      contractDeployerArtifact.bytecode
    );
    contractDeployer = ContractDeployer__factory.connect(
      DEPLOYER_SYSTEM_CONTRACT_ADDRESS,
      wallet
    );

    nonceHolder = NonceHolder__factory.connect(
      NONCE_HOLDER_SYSTEM_CONTRACT_ADDRESS,
      wallet
    );

    let contractDeployerSystemCallContract = await deployContract(
      "SystemCaller",
    ...
    let contractDeployerNotSystemCallContract = await deployContract(
      "NotSystemCaller",
      [contractDeployer.address]
    );
  ...
  describe("updateAccountVersion", function () {
    it("non system call failed", async () => {
      await expect(
        contractDeployer.updateAccountVersion(AA_VERSION_NONE)
    ...
    it("from none to version1", async () => {
      expect(
        (
          await contractDeployer.getAccountInfo(
  ...
  describe("updateNonceOrdering", function () {
    it("non system call failed", async () => {
      await expect(
        contractDeployer.updateNonceOrdering(NONCE_ORDERING_SEQUENTIAL)
      ).to.be.revertedWith("This method require system call flag");
    });
    ...
    it("success from sequential to arbitrary", async () => {
      ...
      await contractDeployerSystemCall.updateNonceOrdering(
    ...
    it("failed from arbitrary to sequential", async () => {
      ...
      await expect(
        contractDeployerSystemCall.updateNonceOrdering(
  ...
  describe("extendedAccountVersion", function () {
    it("account abstraction contract", async () => {
      await contractDeployerSystemCall.updateAccountVersion(AA_VERSION_1);
      expect(
        await contractDeployer.extendedAccountVersion(
          contractDeployerSystemCall.address
        )
      ).to.be.eq(AA_VERSION_1);
      await contractDeployerSystemCall.updateAccountVersion(AA_VERSION_NONE);
    ...
    it("EOA", async () => {
      expect(await contractDeployer.extendedAccountVersion(EOA)).to.be.eq(
        AA_VERSION_1
      );
    });
  ...
  describe("getNewAddressCreate", function () {
    it("success", async () => {
      expect(
        await contractDeployer.getNewAddressCreate(RANDOM_ADDRESS, 3223233)
      ).to.be.eq(utils.createAddress(RANDOM_ADDRESS, 3223233));
    });
  });
  ...
  describe("createAccount", function () {
    ...
    it("successfully deployed", async () => {
      ...
      await expect(
        ...
        .to.emit(
          Deployable__factory.connect(expectedAddress, wallet),
          "Deployed"
        )
  ...
  describe("forceDeployOnAddress", function () {
    it("not from self call failed", async () => {
      ...
      await expect(
        contractDeployer.forceDeployOnAddress(deploymentData, wallet.address)
      ).to.be.revertedWith("Callable only by self");
    });
    ...
    it("not known bytecode hash failed", async () => {
      ...
      await expect(
        contractDeployer
          ...
          .forceDeployOnAddress(deploymentData, wallet.address)
      ).to.be.revertedWith("The code hash is not known");
    });
    ...
    it("successfully deployed", async () => {
      ...
      await expect(
        ...
        .to.not.emit(
          Deployable__factory.connect(RANDOM_ADDRESS, wallet),
          "Deployed"
        );
      let accountInfo = await contractDeployer.getAccountInfo(RANDOM_ADDRESS);
      expect(accountInfo.supportedAAVersion).to.be.eq(AA_VERSION_NONE);
  ...
  describe("forceDeployOnAddresses", function () {
    it("not allowed to call", async () => {
      ...
      await expect(
        contractDeployer.forceDeployOnAddresses(deploymentData)
      ).to.be.revertedWith(
    ...
    it("successfully deployed", async () => {
      ...
      await expect(
        contractDeployer
          .connect(forceDeployer)
          .forceDeployOnAddresses(deploymentData)
      )
        .to.emit(contractDeployer, "ContractDeployed")
        .withArgs(
          forceDeployer.address,
          utils.hashBytecode(deployableArtifact.bytecode),
          RANDOM_ADDRESS_2
        )
        .to.emit(contractDeployer, "ContractDeployed")
        ...
        .to.emit(
          Deployable__factory.connect(RANDOM_ADDRESS_2, wallet),
          "Deployed"
        )
        .withArgs(0, "0x")
        .to.not.emit(
          Deployable__factory.connect(RANDOM_ADDRESS_3, wallet),
          "Deployed"
        );

      let accountInfo1 =
        await contractDeployer.getAccountInfo(RANDOM_ADDRESS_2);
      ...
      let accountInfo2 =
        await contractDeployer.getAccountInfo(RANDOM_ADDRESS_3);
