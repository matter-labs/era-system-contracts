...
import { ContractDeployer, ContractDeployer__factory, NonceHolder, NonceHolder__factory, Deployable__factory } from '../typechain-types';
import { DEPLOYER_SYSTEM_CONTRACT_ADDRESS, NONCE_HOLDER_SYSTEM_CONTRACT_ADDRESS, FORCE_DEPLOYER_ADDRESS } from './shared/constants';
...
describe('ContractDeployer tests', function () {
    ...
    before(async () => {
        ...
        contractDeployerSystemCall = new Contract(contractDeployerSystemCallContract.address, contractDeployerArtifact.abi, wallet) as ContractDeployer

        let contractDeployerNotSystemCallContract = await deployContract('NotSystemCaller', [contractDeployer.address]);
        contractDeployerNotSystemCall = new Contract(contractDeployerNotSystemCallContract.address, contractDeployerArtifact.abi, wallet) as ContractDeployer
    ...
    describe('updateAccountVersion', function () {
        ...
        it('from none to version1', async () => {
            expect((await contractDeployer.getAccountInfo(contractDeployerSystemCall.address)).supportedAAVersion)
    ...
    describe('updateNonceOrdering', function () {
        ...
        it('success from sequential to arbitrary', async () => {
            expect((await contractDeployer.getAccountInfo(contractDeployerSystemCall.address)).nonceOrdering)
    ...
    describe('extendedAccountVersion', function () {
        it('account abstraction contract', async () => {
            await contractDeployerSystemCall.updateAccountVersion(AA_VERSION_1)
            expect(await contractDeployer.extendedAccountVersion(contractDeployerSystemCall.address))
                .to.be.eq(AA_VERSION_1)
            await contractDeployerSystemCall.updateAccountVersion(AA_VERSION_NONE)
        });

        it('EOA', async () => {
            expect(await contractDeployer.extendedAccountVersion(EOA))
                .to.be.eq(AA_VERSION_1)
        });
    ...
    describe('getNewAddressCreate2', function () {
        it('success', async () => {
            expect(await contractDeployer.getNewAddressCreate2(
                RANDOM_ADDRESS,
                ...
                .to.be.eq(utils.create2Address(RANDOM_ADDRESS,
                '0x0100FFFFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEF',
                '0x0000000022000000000123812381283812831823812838912389128938912893',
                '0x'))
        });
    ...
    describe('getNewAddressCreate', function () {
        it('success', async () => {
            expect(await contractDeployer.getNewAddressCreate(
                RANDOM_ADDRESS,
                3223233
            ))
                .to.be.eq(utils.createAddress(RANDOM_ADDRESS,
                3223233))
        });
    });
    ...
    describe('createAccount', function () {
        it('non system call failed', async () => {
            await expect(contractDeployerNotSystemCall.createAccount(
        ...
        it('zero bytecode hash failed', async () => {
            await expect(contractDeployerSystemCall.createAccount(
                ethers.constants.HashZero,
                ethers.constants.HashZero,
                '0x',
                AA_VERSION_NONE
            )).to.be.revertedWith(
                'BytecodeHash cannot be zero'
            );
        });
        ...
        it('successfully deployed', async () => {
            ...
            await expect(contractDeployer.createAccount(
                ethers.constants.HashZero,
                utils.hashBytecode(deployableArtifact.bytecode),
                '0xdeadbeef',
                AA_VERSION_NONE
            )).to.emit(
                contractDeployer, 'ContractDeployed'
            ).withArgs(wallet.address, utils.hashBytecode(deployableArtifact.bytecode), expectedAddress)
            .to.emit(
                    Deployable__factory.connect(expectedAddress, wallet), 'Deployed'
    ...
    describe('create2Account', function () {
        it('non system call failed', async () => {
            await expect(contractDeployerNotSystemCall.create2Account(
        ...
        it('zero bytecode hash failed', async () => {
            await expect(contractDeployerSystemCall.create2Account(
                '0x1234567891234567891234512222122167891123456789123456787654323456',
                ethers.constants.HashZero,
                '0x',
                AA_VERSION_NONE
        ...
        it('successfully deployed', async () => {
            let expectedAddress = utils.create2Address(wallet.address, utils.hashBytecode(deployableArtifact.bytecode), '0x1234567891234567891234512222122167891123456789123456787654323456', '0xdeadbeef')
            await expect(contractDeployer.create2Account(
                '0x1234567891234567891234512222122167891123456789123456787654323456',
                utils.hashBytecode(deployableArtifact.bytecode),
                '0xdeadbeef',
                AA_VERSION_NONE
        ...
        it('non-zero value deployed', async () => {
            let expectedAddress = utils.create2Address(wallet.address, utils.hashBytecode(deployableArtifact.bytecode), ethers.constants.HashZero, '0x')
            await expect(contractDeployer.create2Account(
                ethers.constants.HashZero,
                utils.hashBytecode(deployableArtifact.bytecode),
                '0x',
    ...
    describe('create', function () {
        it('non system call failed', async () => {
            await expect(contractDeployerNotSystemCall.create(
                ethers.constants.HashZero,
                utils.hashBytecode(deployableArtifact.bytecode),
                '0x'
            )).to.be.revertedWith(
        ...
        it('successfully deployed', async () => {
            let nonce = await nonceHolder.getDeploymentNonce(wallet.address)
            let expectedAddress = utils.createAddress(wallet.address, nonce)
            await expect(contractDeployer.create(
                ethers.constants.HashZero,
                utils.hashBytecode(deployableArtifact.bytecode),
                '0x12'
            )).to.emit(
    ...
    describe('create2', function () {
        it('non system call failed', async () => {
            await expect(contractDeployerNotSystemCall.create2(
                ethers.constants.HashZero,
                utils.hashBytecode(deployableArtifact.bytecode),
                '0x'
            )).to.be.revertedWith(
        ...
        it('successfully deployed', async () => {
            let expectedAddress = utils.create2Address(wallet.address, utils.hashBytecode(deployableArtifact.bytecode), '0x1234567891234567891234512222122167891123456789123456787654323456', '0xab')
            await expect(contractDeployer.create2(
                '0x1234567891234567891234512222122167891123456789123456787654323456',
                utils.hashBytecode(deployableArtifact.bytecode),
                '0xab'
            )).to.emit(
    ...
    describe('forceDeployOnAddress', function () {
        it('not from self call failed', async () => {
            ...
            await expect(contractDeployer.forceDeployOnAddress(
                deploymentData,
                wallet.address
            )).to.be.revertedWith(
        ...
        it('not known bytecode hash failed', async () => {
            ...
            await expect(contractDeployer.connect(deployerAccount).forceDeployOnAddress(
                deploymentData,
                wallet.address
            )).to.be.revertedWith(
                'The code hash is not known'
        ...
        it('successfully deployed', async () => {
            ...
            await expect(contractDeployer.connect(deployerAccount).forceDeployOnAddress(
                ...
                .to.not.emit(
                    Deployable__factory.connect(RANDOM_ADDRESS, wallet), 'Deployed'
                )
            let accountInfo = await contractDeployer.getAccountInfo(RANDOM_ADDRESS)
            expect(accountInfo.supportedAAVersion).to.be.eq(AA_VERSION_NONE)
            expect(accountInfo.nonceOrdering).to.be.eq(NONCE_ORDERING_SEQUENTIAL)
    ...
    describe('forceDeployOnAddresses', function () {
        it('not allowed to call', async () => {
            ...
            await expect(contractDeployer.forceDeployOnAddresses(
                deploymentData
            )).to.be.revertedWith(
        ...
        it('successfully deployed', async () => {
            ...
            await expect(contractDeployer.connect(forceDeployer).forceDeployOnAddresses(
                deploymentData
            )).to.emit(
                contractDeployer, 'ContractDeployed'
            ...
            .to.emit(
                Deployable__factory.connect(RANDOM_ADDRESS_2, wallet), 'Deployed'
            )
                .withArgs(0, '0x')
            .to.not.emit(
                Deployable__factory.connect(RANDOM_ADDRESS_3, wallet), 'Deployed'
            )
