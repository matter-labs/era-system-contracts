...
xdescribe("DefaultAccount tests", function () {
  ...
  before(async () => {
    ...
    l2EthToken = L2EthToken__factory.connect(
      ETH_TOKEN_SYSTEM_CONTRACT_ADDRESS,
      wallet
    );
    callable = (await deployContract("Callable")) as Callable;
    mockERC20Approve = (await deployContract(
      "MockERC20Approve"
    )) as MockERC20Approve;

    let paymasterFlowInterfaceArtifact = await loadArtifact("IPaymasterFlow");
    paymasterFlowInterface = new ethers.utils.Interface(
      paymasterFlowInterfaceArtifact.abi
    );
  ...
  describe("validateTransaction", function () {
    it("non-deployer ignored", async () => {
      ...
      const call = {
        ...
        value: 0,
        data: defaultAccount.interface.encodeFunctionData(
          "validateTransaction",
          [txHash, signedHash, txData]
        ),
      };
    ...
    it("invalid ignature", async () => {
      ...
      const parsedTx = zksync.utils.parseTransaction(txBytes);
      parsedTx.s =
        "0x0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0";
      const txData = signedTxToTransactionData(parsedTx)!;
    ...
    it("valid tx", async () => {
      ...
      expect(await bootloader.provider.call(call)).to.be.eq(
        defaultAccount.interface.getSighash("validateTransaction") +
          "0".repeat(56)
      );
    });
  });
  ...
  describe("executeTransaction", function () {
    it("non-deployer ignored", async () => {
      ...
      await expect(
        await defaultAccount.executeTransaction(txHash, signedHash, txData)
    ...
    it("successfully executed", async () => {
      ...
      await expect(
        await defaultAccount
          .connect(bootloader)
          .executeTransaction(txHash, signedHash, txData)
      )
  ...
  describe("executeTransactionFromOutside", function () {
    it("nothing", async () => {
      ...
      await expect(
        await defaultAccount.executeTransactionFromOutside(txData)
      ).to.not.emit(callable, "Called");
    });
  });
  ...
  describe("payForTransaction", function () {
    ...
    it("successfully payed", async () => {
      ...
      await expect(
        await defaultAccount
          .connect(bootloader)
          .payForTransaction(txHash, signedHash, txData)
      )
  ...
  describe("prepareForPaymaster", function () {
    it("non-deployer ignored", async () => {
      const eip712Tx = await account.populateTransaction({
        ...
        customData: {
          ...
          paymasterParams: {
            ...
            paymasterInput: paymasterFlowInterface.encodeFunctionData(
              "approvalBased",
              [mockERC20Approve.address, 2023, "0x"]
            ),
          },
        },
      });
      ...
      await expect(
        await defaultAccount.prepareForPaymaster(
          eip712TxHash,
          eip712SignedHash,
          eip712TxData
        )
