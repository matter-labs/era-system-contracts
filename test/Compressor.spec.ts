...
describe("Compressor tests", function () {
  ...
  describe("publishCompressedBytecode", function () {
    ...
    it("successfully published", async () => {
      ...
      await mockl1Messenger.setResult(l1MessengerIface.encodeFunctionData("sendToL1", [COMPRESSED_BYTECODE]), {
        failure: false,
      ...
      await expect(compressor.connect(bootloaderAccount).publishCompressedBytecode(BYTECODE, COMPRESSED_BYTECODE))
        .to.emit(mockKnowCodesStorage, "Called")
        .withArgs(
          0,
          knownCodesStorageIface.encodeFunctionData("markBytecodeAsPublished", [zksync.utils.hashBytecode(BYTECODE)])
        );
    });
  });
