...
import { BOOTLOADER_FORMAL_ADDRESS, EMPTY_STRING_KECCAK, L1_MESSENGER_SYSTEM_CONTRACT_ADDRESS, COMPRESSOR_CONTRACT_ADDRESS } from './shared/constants';
...
describe('KnownCodesStorage tests', function () {
    ...
    describe('markFactoryDeps', function () {
        ...
        it('incorrectly fomatted bytecode hash failed to call', async () => {
            await expect(
                knownCodesStorage.connect(bootloaderAccount).markFactoryDeps(true, [BYTECODE_HASH_2, INCORRECTLY_FORMATTED_HASH])
            ).to.be.revertedWith('Incorrectly formatted bytecodeHash');
        });
        ...
        it('successfuly marked', async () => {
            await expect(knownCodesStorage.connect(bootloaderAccount).markFactoryDeps(false, [BYTECODE_HASH_2, BYTECODE_HASH_3]))
                .to.emit(knownCodesStorage, 'MarkedAsKnown')
    ...
    describe('getMarker', function () {
        ...
        it('known', async () => {
            expect(
                await knownCodesStorage.getMarker(BYTECODE_HASH_1)
            ).to.be.eq(1);
        });
    });
});
