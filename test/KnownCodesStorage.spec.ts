...
xdescribe("KnownCodesStorage tests", function () {
  ...
  const BYTECODE_HASH_1 =
    "0x0100FFFFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEF";
  const BYTECODE_HASH_2 =
    "0x0100FFFFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEE1";
  const BYTECODE_HASH_3 =
    "0x0100FFFFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEE2";
  const BYTECODE_HASH_4 =
    "0x0100FFFFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEE3";
  const INCORRECTLY_FORMATTED_HASH =
    "0x0120FFFFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEF";
  const INVALID_LENGTH_HASH =
    "0x0100FFFEDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEFDEADBEEF";

  before(async () => {
    wallet = (await getWallets())[0];
    knownCodesStorage = (await deployContract(
      "KnownCodesStorage"
    )) as KnownCodesStorage;
    ...
    await setCode(
      L1_MESSENGER_SYSTEM_CONTRACT_ADDRESS,
      l1MessengerArtifact.bytecode
    );
    mockL1Messenger = MockL1Messenger__factory.connect(
      L1_MESSENGER_SYSTEM_CONTRACT_ADDRESS,
      wallet
    );
  ...
  describe("markBytecodeAsPublished", function () {
    it("non-compressor failed to call", async () => {
      await expect(
        knownCodesStorage.markBytecodeAsPublished(BYTECODE_HASH_1)
      ).to.be.revertedWith("Callable only by the compressor");
    });

    it("incorrectly fomatted bytecode hash failed to call", async () => {
      await expect(
        knownCodesStorage
          .connect(compressorAccount)
          .markBytecodeAsPublished(INCORRECTLY_FORMATTED_HASH)
    ...
    it("invalid length bytecode hash failed to call", async () => {
      await expect(
        knownCodesStorage
          .connect(compressorAccount)
          .markBytecodeAsPublished(INVALID_LENGTH_HASH)
    ...
    it("successfuly marked", async () => {
      await expect(
        knownCodesStorage
          .connect(compressorAccount)
          .markBytecodeAsPublished(BYTECODE_HASH_1)
      )
  ...
  describe("markFactoryDeps", function () {
    it("non-bootloader failed to call", async () => {
      await expect(
        knownCodesStorage.markFactoryDeps(false, [
          BYTECODE_HASH_2,
          BYTECODE_HASH_3,
        ])
    ...
    it("successfuly marked", async () => {
      await expect(
        knownCodesStorage
          .connect(bootloaderAccount)
          .markFactoryDeps(false, [BYTECODE_HASH_2, BYTECODE_HASH_3])
      )
        .to.emit(knownCodesStorage, "MarkedAsKnown")
    ...
    it("sent to l1", async () => {
      await expect(
        knownCodesStorage
          .connect(bootloaderAccount)
          .markFactoryDeps(true, [BYTECODE_HASH_4])
      )
        .to.emit(knownCodesStorage, "MarkedAsKnown")
  ...
  describe("getMarker", function () {
    it("not known", async () => {
      expect(
        await knownCodesStorage.getMarker(INCORRECTLY_FORMATTED_HASH)
      ).to.be.eq(0);
    });
