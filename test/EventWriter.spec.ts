...
describe('EventWriter tests', function () {
    ...
    it('zero topics', async () => {
        const txResponse = await extraAbiCaller.fallback({data: encodeExtraAbiCallerCalldata(REAL_EVENT_WRITER_CONTRACT_ADDRESS, 0, [0], '0x')})
        expect(await checkReturnedEvent(
            txResponse,
    ...
    it('one topic', async () => {
        const txResponse = await extraAbiCaller.fallback({data: encodeExtraAbiCallerCalldata(REAL_EVENT_WRITER_CONTRACT_ADDRESS, 0, [
    ...
    it('two topics', async () => {
        const txResponse = await extraAbiCaller.fallback({data: encodeExtraAbiCallerCalldata(REAL_EVENT_WRITER_CONTRACT_ADDRESS, 0, [
                2, '0x1234567890123456789012345678901234567890123456789012345678901234', '0x1278378123784223232874782378478237848723784782378423747237848723'
            ], '0xabcd')})
        expect(await checkReturnedEvent(
            txResponse,
            extraAbiCaller.address,
            ['0x1234567890123456789012345678901234567890123456789012345678901234', '0x1278378123784223232874782378478237848723784782378423747237848723'],
            '0xabcd'
        )).to.be.eq(true)
    });
    ...
    it('three topics', async () => {
        const txResponse = await extraAbiCaller.fallback({data: encodeExtraAbiCallerCalldata(REAL_EVENT_WRITER_CONTRACT_ADDRESS, 0, [
                3, '0x1234567890123456789012345678901234567890123456789012345678901234', '0x1278378123784223232874782378478237848723784782378423747237848723', ethers.constants.HashZero
            ], '0x')})
        expect(await checkReturnedEvent(
            txResponse,
            extraAbiCaller.address,
    ...
    it('four topics', async () => {
        const txResponse = await extraAbiCaller.fallback({data: encodeExtraAbiCallerCalldata(REAL_EVENT_WRITER_CONTRACT_ADDRESS, 0, [
                4, ONE_BYTES32_HEX, '0x1234567890123456789012345678901234567890123456789012345678901234', '0x1278378123784223232874782378478237848723784782378423747237848723', ethers.constants.HashZero
            ], '0x2828383489438934898934893894893895348915893489589348958349589348958934859348958934858394589348958934854385838954893489')})
        expect(await checkReturnedEvent(
            txResponse,
            extraAbiCaller.address,
            [ONE_BYTES32_HEX, '0x1234567890123456789012345678901234567890123456789012345678901234', '0x1278378123784223232874782378478237848723784782378423747237848723',
                ethers.constants.HashZero],
            '0x2828383489438934898934893894893895348915893489589348958349589348958934859348958934858394589348958934854385838954893489'
        )).to.be.eq(true)
    });
    ...
    it('five topics failed', async () => {
        await expect(extraAbiCaller.fallback({data: encodeExtraAbiCallerCalldata(REAL_EVENT_WRITER_CONTRACT_ADDRESS, 0, [
                5, ONE_BYTES32_HEX, ONE_BYTES32_HEX, ONE_BYTES32_HEX, ONE_BYTES32_HEX, ONE_BYTES32_HEX
            ], '0x')})).to.be.reverted;
    });
});

async function checkReturnedEvent(txResponse: TransactionResponse, address: string, topics: string[], data: string): boolean {
    const receipt = await txResponse.wait()
    const eventsFromAddress = receipt
        .logs
