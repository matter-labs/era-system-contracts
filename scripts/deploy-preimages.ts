...
class ZkSyncDeployer {
  ...
  async shouldUpgradeSystemContract(contractAddress: string, expectedBytecodeHash: string): Promise<boolean> {
    ...
    const currentBytecodeHash = await this.deployer.zkWallet.provider.getStorage(
  ...
  async publishDependencies(dependenciesToPublish: Dependency[]) {
    ...
    for (const dependency of dependenciesToPublish) {
      const dependencyLength = dependency.bytecodes.reduce(
        (prev, dep) => prev + ethers.toBeArray(ethers.hexlify(dep)).length,
        0
      );
  ...
  program
    ...
    .action(async (cmd) => {
      ...
      const wallet = cmd.privateKey
        ...
        : Wallet.fromMnemonic(process.env.MNEMONIC ? process.env.MNEMONIC : ethTestConfig.mnemonic);
      ...
      const gasPrice = cmd.gasPrice
        ...
        : (await providerL1.getFeeData()).gasPrice;
